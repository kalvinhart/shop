import { InputConfig } from "../components/AuthForm/hooks/useAuthForm";
...
export const validateForm = (formValues: InputConfig) => {
  ...
  for (const field in formValues) {
    if (
      formValues[field as keyof InputConfig]!.options.required &&
    ...
    if (
      ...
      formValues[field as keyof InputConfig]!.value !== "" &&
      formValues[field as keyof InputConfig]!.value.length <
        formValues[field as keyof InputConfig]!.options.min!
    ) {
      errors[field as keyof Errors] = `This field must contain a minimum of ${
        formValues[field as keyof InputConfig]!.options.min
      } characters.`;
    }
  }
